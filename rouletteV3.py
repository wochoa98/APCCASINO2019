...
rounds = 5
tableMin = 10 #need to delete later and only use tableMin from Roulette class
#for i in rounds:
class player:
    ...
    def __init__(self, num, budget):
        ...
        self.bet = random.randint(tableMin, budget)
...
class rouletteGame:
    playerList = []
    casinoWinnings = 0 #ADD VALUES HERE WHEN PLAYER LOSES
    result = 0
    tableMin = 10
    ...
    def roll(self):
        self.result = random.choice(straight_up)
    ...
    def player_bet_balance(player):
        ...
        player.balance = player.budget - player.bet
        print("Player", player.playerNumber, "has", player.budget,"dollars and bets", player.bet, "dollars leaving them with", player.balance)
            ...
            if p.playtype == 3:
                if p.bet <= p.budget/3:
                    ...
                    if y == 36:
                        ...
                        print("you ", p.playerNumber,"can't bet on zero")
                    else:
                        z.append(y)
                        z.append(y+1)
                    if self.result in z:
